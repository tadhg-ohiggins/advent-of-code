...
def raw_to_grid(raw):
    return make_grid(splitstriplines(raw))
...
def flash(grid, flash_count):
    flashed = []
    for k in grid:
        if grid[k] > 9 and k not in flashed:
            flashed = flashed + [k]
            grid, flashed = incr_neighbors(grid, k, flashed)

    for pt in flashed:
        grid[pt] = 0
    return grid, flash_count + len(flashed)


def grid_step(grid, flash_count):
    grid = valmap(lambda x: x + 1, grid)
    return flash(grid, flash_count)


def process_one(grid):
    flash_count = 0
    for i in range(100):
        grid, flash_count = grid_step(grid, flash_count)
    return flash_count
...
def process_two(grid):
    ...
    while True:
        count = count + 1
        grid, flash_count = grid_step(grid, 0)
        if len(grid) == flash_count:
            return count
