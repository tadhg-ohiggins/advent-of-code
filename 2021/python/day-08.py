...
def decipher_line(line):
    ...
    known = keyfilter(
        ...
        {NUM_SEGS.get(len(d)): d for d in value + signals},
    )
    # If it's in 7 not 1 it's a.
    decipherer["a"] = (set(known[7]) - set(known[1])).pop()
    ...
    decipherer["f"] = (set(known[1]) - set(ac)).pop()
    assert len(decipherer.keys()) == len(set(decipherer.values()))
    ...
    decipherer["b"] = (set(known[6]) - seven_u_two).pop()
    assert len(decipherer.keys()) == len(set(decipherer.values()))

    # Whatever isn't these in four is d:
    four_not_bcf = set(known[4]) - set(
        pick(["b", "c", "f"], decipherer).values()
    )
    assert len(four_not_bcf) == 1
    decipherer["d"] = four_not_bcf.pop()
    assert len(decipherer.keys()) == len(set(decipherer.values()))

    # eight minus d is zero:
    known[0] = "".join(
        sorted(filter(lambda x: x != decipherer["d"], known[8]))
    )
    ...
    # Whatever's in the signal that isn't in decipherer values must be g:
    leftover = set(all_chars) - set(decipherer.values())
    assert len(leftover) == 1
    decipherer["g"] = leftover.pop()
