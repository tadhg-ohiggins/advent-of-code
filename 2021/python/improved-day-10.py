...
def process_one(data):
    return sum(map(find_errors, data))


def line_openers(line: list, stack: list) -> list:
    if not len(line):
        return stack
    char, rest = line[0], line[1:]
    if char in openers:
        return line_openers(rest, stack + [char])
    if stack and closers.get(char) == stack[-1]:
        return line_openers(rest, stack[:-1])
    return line_openers(rest, stack)


def xline_openers(line: list, stack: list) -> list:
    if not len(line):
        return stack
    char, rest = line[0], line[1:]
    opener = char in openers
    closer = stack and get_o_for_c.get(char) == stack[-1]
    if opener:
        return line_openers(rest, stack + [char])
    if closer:
        return line_openers(rest, stack[:-1])
...
def complete_line(line) -> int:
    ...
    """
    stack = line_openers(list(line), [])
    closing = map(openers.get, stack[::-1])
