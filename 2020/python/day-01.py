...
if __name__ == "__main__":
    raw = filter(None, Path("input-01.txt").read_text().splitlines())
    data = list(map(int, raw))
    print("part one:", find_totals(2020, data))

    for item in data:
        xx = find_totals(2020 - item, data)
        if xx:
            result = xx * item
            break
            # print(xx * item)
...
Your puzzle answer was 357504.  --- Part Two ---
